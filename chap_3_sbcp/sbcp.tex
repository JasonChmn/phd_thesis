...
We now propose to explore the following question: \textit{What is a \textit{good} guide path to generate a valid contact sequence ?}
...
In this chapter, we use an acyclic Sampling Based Contact Planner \cite{AcyclicCP} for multiped robot locomotion, which for convenience we will refer to as \qq{SBCP}.
% What do we do ?
%To do so, we use SBCP as a black-box during the learning of LEAS, to validate the trajectories generated following our Master-Worker architecture.
%Given a guide path in input, a sequence of root configurations in $SE(3)$, SBCP computes a sequence of whole body configurations in contact with the terrain along it (figure \ref{fig:cp-sb:strategy_cp-sb}).
...
In the context of Motion-before-Contact, LEAS learns to address by reinforcement one of the key limitations of this strategy: the non-guarantee of feasibility of the guide (P1) with a contact planner (P2).
...
Section \ref{sub:cp-sb:leas_coupling} shows how to train LEAS with SBCP as a black-box. 
...
The Sampling-based Contact Planner (SBCP) \cite{AcyclicCP} belongs to the motion-before-contact family of contact planners, using a rough robot root trajectory (guide path) to generate the contacts along.
...
While SBCP can perform multi-contact locomotion tasks such as standing up or climbing stairs using a handrail, in this work we are interested in biped walking for the scenarios tested in Chapter \ref{sec:LEAS} where we only enable foot contacts with the terrain.
...
% In this section we present an overview of SBCP, then we expose the feasiblity problem with the guide path planner (P1).
...
Given an initial whole body configuration $q^0$, the role of SBCP is to populate a guide path in input $G = [q_{base}^0,q_{base}^1,..., q_{base}^{M-1}]$ with a sequence of configurations in contact double support with the terrain and static equilibrium [$q^0$,$q^1$,..., $q^{M'-1}$] following \textbf{exactly} the guide path. 
% Comments steve
...
First, an offline database is generated containing configurations $q_k$ for each limb $k$. %to create contacts.
...
Starting from a robot configuration $q^i$ in contact whose root is at $q_{base}^i$, the contact planner moves the robot root to the next position on the guide $q_{base}^{i+1}$. To do so, SBCP performs the following steps (See figure \ref{fig:overview_sbcp}):
\begin{itemize}
...
%\stn{pourquoi cette section n arrive pas bcp plus tôt, qu on sache pourquoi on lit les specs du planner ? }
% We pre-selected the parameters to obtain a decent computation time + solution with as few configurations as possible considering the success rate on our previous methods.
% Now we use SBCP as a blackbox that takes as input the guide path and returns a contact sequence on it, up to the last feasible root configuration.
% Problems:
% SBCP follows exactly the guide => Problem when the guide is bad.
% We do not know what is a "good" guide that works well with SBCP and its user-defined heuristics other than empirically.
% We want to have a steering method that learns it automatically in order to succeed with SBCP.
...
% Our conclusion
Our experiments have shown that the major factor impacting the success rate of SBCP is the guide path in input.
...
As a consequence, it is difficult to define new additional constraints or heuristics to our guide path generator ($P1$) to better approximate the feasibility of SBCP ($P2$).

Our solution to solve such feasibility problem is to learn with LEAS how to validate its guide paths with SBCP, which computes the contact sequence along them. In other words, we want LEAS to answer the question: \textit{What is a good guide path for this contact planner ?}
...
%\stn{on a aucune idee de pourquoi on etudie ces parametres a ce stade. il faut expliquer la problematique d abord et ensuite expliquer, dans tes resultats, comment les parametres influents}
%\textcolor{blue}{Ces parametres sont la pour comprendre le trade-off et pourquoi ca rate du coup. Si on prend une tres petite discretization avec beaucoup de samples, on va quasiment toujours reussir.}
...
A trade-off has to be found on these values to comply with the requirements on the computation time and the success rate of the contact planner, but also to propose a fair comparison of LEAS with our previous steering methods on SBCP.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Analysis of the success rate and computation time of SBCP with a guide path computed by RB-Kino in function of: (a) the number of samples $N$ and (b) the guide path discretization $\Delta D$ (results averaged over 3 random seeds).}
...
The success of a guide path is defined as the percentage of the guide successful with SBCP. For example for a guide path of 100 configurations, if the contact planner from the configuration of index 49 fails to reach index 50 then the success value is $50$\%.
...
We then evaluate the results of different sample values $N$ for 3 different seeds, with a guide path discretization step $\Delta D = 2$cm. We will explain the choice of $\Delta D$ later on in this section.
...
We compare these results to the original paper \cite{AcyclicCP} 
%where, with a number of samples $N=100$K 
on the robot HRP-2 that is similar in maximum step size to the robot Talos we work on, where they state that "In most scenarios the torso of HRP-2 moves about 15 cm (for easy scenarios) between two postures, but only 3 cm for the car egress scenario". 
% Results
From our measurements, we see that a small discretization step $\Delta D=2$cm on the guide increases the success rate of the contact planner. Indeed, it is easier for the robot to create a contact to reach a closer root configuration than a distant one.
%where we risk to break contacts with both legs, thus leading to a repositioning of the legs and can still fail to reach the next root position on the guide. \textcolor{red}{A small picture could be nice, to show the discretization of the guide.}
% But a high discretization can also lead to a contact planning of bad quality, a lot of unnecessary foosteps and result very dependant on the heuristics as well (robustness score good but bad visually) => It moves with one foot in front and the other behind and the feet order stays the same during all the walk, always at the limit with the foot behind to balance the foot that is a lot in front. 
...
\section{Implementation design\label{sub:cp-sb:leas_coupling}}
...
States generated by LEAS are subject to some reachability $\tilde{\mathcal{R}^*}$ and collision-free $\tilde{\mathcal{C}}$ conditions (see section \ref{subsub:validity_approximation}) \textcolor{blue}{Reference to chapter LEAS}, plus a new additional constraint on the whole guide path that is to succeed with the contact planner that we will explain later on.
...
Learning curves of LEAS without contact planner, referred as LEAS-P1 (Chapter \ref{sec:LEAS}), and LEAS with the contact planner, LEAS-P2, are shown in Figure \ref{fig:cp-sb:learning_curves_p1_p2} (the maximum reward is equal to 100 for trajectories in a straight line). 
...
The success rate presented in the results corresponds to the percentage of valid guide paths reaching the goal that are successful with SBCP.
...
RB-Lin rotates at a fixed angular velocity $\omega_{max}$ which is equal to a maximum rotation of $\omega_{max} \times T \times A = \frac{\pi}{9}$ radians between each configurations on the guide. As expected, the results show that for any initial orientation, rotating the robot with a step angle of $\frac{\pi}{9}$ before moving it to the goal generates guide paths succeeding with SBCP. 
...
In our tests, all step angles in the range $]0,\frac{\pi}{2}]$ were successful on SBCP, but in practice, a step angle of $\frac{\pi}{4}$ may be the maximum feasible in reality by the Talos robot.
...
In this work, we use the measure of the $robustness$ to give us an overall estimation of the stability of the contact plan generated, where a lower value means a more difficult guide path in the input of SBCP (i.e. too close to the obstacles, too high or low...) leading to unstable configurations in contact.
...
Results are shown in Table \ref{tab:cp-sb:hole_bridge} with the success rate of SBCP for both bridge and hole scenarios. Figure \ref{fig:cp-sb:trajectories_hole} shows generated trajectories on the hole scenario.


% RB-Kino and RB-Lin
Results show that RB-Lin and RB-Kino generate valid guide path up to the goal, but do not consider the terrain, resulting in guide paths passing close to the void. As a consequence, these guide paths lead to a low success rate and some weakly robust contact plans on both terrains.
...
In contrast, the results of LEAS-P2 demonstrate how it can produce guide paths staying even further from the hole, thus generating more robust contact configurations and increasing its success rate with SBCP.
...
However, we also notice that this correlation is not pertinent for all scenarios as the bridge where RB-Kino presents a high robustness score close to LEAS-P2 but the worst success rate among our steering methods with SBCP. 
...
We now evaluate on the wall scenario what kind of additional constraints LEAS can learn from SBCP. In the wall scenario, LEAS has to generate a collision-free guide path ($\tilde{\mathcal{C}}$).
...
In the seminal paper presenting SBCP \cite{AcyclicCP}, these two scenarios were solved easily with the steering methods RB-Lin and RB-Kino with a near $100$ \% success rate. 
...
In this work, we extend these tests on a wider range of initial configurations with RB-Kino (with RRT path planning if required), LEAS-P1 and LEAS-P2. Then we measure their success rate with SBCP and the robustness of the contact plans generated when crossing the terrain.
...
% Success rate
Results in Table \ref{tab:cp-sb:rubbles_stairs} show that the success of RB-Kino+RRT suffers from a limitation, also hinted by with RB-Kino without RRT, where guide paths with configurations at the limit of the reachability condition $\mathcal{R}$ can lead to a failed contact plan with SBCP (magenta dots on Figure \ref{fig:cp-sb:stairs_sm}).
...
% Conclusion:
% LEAS can improve the success rate of the CP-SB, generating guides fitting this contact planner. It alleviates the limitations of CP-SB by learning how to help its heuristics.
...
% A - Talk about when I did not set any reward on the orientation => Sidewalking.
% B - What we know: Rotate first, then move + stay away from the void + keep a constant distance from the ground.
% C - Discretization on the guide: selected at 10, but could be interesting to go higher + we skip some configurations along the guide with LEAS, other solutions are possible as just increasing the timestep, but it would it would be less stable than our actual implementation and may require a bit of reward engineering. => Insanely long to train as there is a lot of repositioning.
% F - Redesign the ROM for locomotion task.
\begin{figure}[h]
    \centering
...
In order to obtain a policy pushing this discretization as much as possible while succeeding the contact planning and at the same time, reducing the computation and increasing the quality of the contact plan, we rewarded LEAS for being close to the maximum $\Delta D$ value.
...
However, such pretraining from demonstrations leads to the so-called \textit{catastrophic forgetting} problem, losing the previously learned experience and starting to relearn from scratch during the fine-tuning, even worsening the results compared to learning from scratch.
