...
Our contribution in this thesis aims at addressing this very complexity of the locomotion problem, where the decision and continuous motion variables and discrete contact locations have to be taken from an intricate process.
...
In order to use gradient-based algorithms, finite-differencing can typically use to approximate the gradients such as the physics engine MuJoCo \cite{mujoco}. However, it tends to introduce round-off and discretization errors.
...
The \textit{contact-before-motion} approach can lead to local minima on short horizon planning, and become computationally intractable on a longer horizon.
