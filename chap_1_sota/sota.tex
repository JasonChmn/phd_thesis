...
While no definitive answer yer exists, several advances have been made on a part of the problem main difficulties.
...
Avoiding collisions between the robot body and the terrain is the first necessary condition for contact planning. 
...
As a refinement to basic obstacle avoidance, the fine details of the terrain map can be approximated by a so called \textit{traversability} score. This technic has been employed in particular during the DARPA challenge \cite{darpa_hutter_2022}. 

\textcolor{blue}{Detailler plus les contrib en dessous.}
To locomote quadruped robots on rough terrains, heuristics have been used to evaluate the likelihood for a body position to find feasible contacts \cite{kolter_2008, terrain_map_mrinal_2011}.
\textcolor{blue}{Peut etre coller ca a la section d'en dessous vu que c'est les memes approches.}

% Grid => kolter / winkler / Arain 2013 / Wermelinger 2016
Works with the same approach \cite{winkler_2014, wermelinger_2016} discretizes the terrain into a body cost map (grid), then searched to plan a guide path. As a result, such paths are more likely to be feasible by a contact planner.
...
\textcolor{blue}{Added by nico}
This leads to high success rate for producing a sequence of whole-body posture, but turn out to be less successful when trying to connect these postures with feasible dynamic trajectories.
...
Combining reachability condition and foothold estimator in \cite{rough_terrain_reachability_hutter_2021},  a foothold score predictor is learned in a supervised fashion. This predictor is then used to remove geometry from the reachability space.
It is then proposed to employ the reachability condition with a PRM and linear interpolation to plan guide paths in complex terrains (Figure \ref{fig:legged_nav_1}).
...
The reachability condition is the cornerstone to build contact planners that go beyond simple traversability reasoning. Yet, it has limited performances as it implies hard-tuned heuristics models that are difficult to generalize, and fail to capture the connectivity between successive key whole-body postures. Supervised learning seems a direction to extend it, but the evident lack of dataset makes it difficult.
